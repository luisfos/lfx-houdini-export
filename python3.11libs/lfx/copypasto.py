...
def copy_asData(selection: list = None, compression: bool = False) -> None:
    ...
    if selection is None:
        ...
        data = hou.data.selectedItemsAsData()
    else:
        data = hou.data.itemsAsData(selection)
    ...
    if compression:
        ...
        compressed_data = zlib.compress(
            json.dumps(data, separators=(",", ":")).encode("utf-8"), level=9
        )
...
def paste_asData() -> None:
    ...
    hou.data.createItemsFromData(target_parent, data, offset_position=offset)
